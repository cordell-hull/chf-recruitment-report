...
export class PDFGenerator {
  ...
  async generate(report, onProgress = () => {}) {
    ...
    this.drawField('(1) Name of teacher', `${report.teacherFirstName} ${report.teacherLastName}`);
    ...
    this.drawField('(4) Place – Country – Platform', report.interviewPlace);
    ...
    if (report.certification.teacherCost) {
      const timing = report.certification.costTiming === 'advance' ? 'pay in advance' : 'pay after arrival';
      this.drawField('Teacher cost', `${report.certification.costAmount} (${timing})`);
    } else {
